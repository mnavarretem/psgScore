...
st_ctrl.SaveBut	= uicontrol(st_panelMain.Ctrl,...
                'Style','pushbutton',...
                'BackgroundColor',st_figSet.BackColor,...
                'HorizontalAlignment','center',...
                'FontSize',st_figSet.TextCtrlSize,...
...
set(get(st_ctrlExtra.AxesEMGsd,'YLabel'),'String','EMG_S_D')     
...
%::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    function fn_file_load(~,~)
        ...
        nm_resp	= exist('psgHystory.mat','file');
        ch_path	= [];
        
        if logical(nm_resp)
            try 
            ch_path = load('psgHystory.mat');
...
%::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    function fn_display_drawlines()
        ...
        vt_evId	= st_hyp.arousals >= st_dat.time{1}(st_disp.posBeg) & ...
                st_hyp.arousals <= st_dat.time{1}(st_disp.posEnd);
        ...
        for kk = 1:numel(vt_evId)
            st_hLines.ArsLines(kk)	= line(...
                                    'Xdata',st_hyp.arousals(vt_evId(kk),:),...
...
%::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    function fn_display_drawspectrum(~,~)
        ...
        vt_limits(2)    = 0.9*max(mx_spectrumCh(:)) * ...
...
%::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
    function fn_display_drawlongterm()
        ...
        if isfield(st_longTerm,'EM')
            ...
            st_hLines.LTLines(9)	= line(...
                                    'Xdata',st_longTerm.time,...
                                    'Ydata',st_longTerm.EM,...
                                    'Parent',st_ctrlExtra.AxesBPM);                                
        end
        if isfield(st_longTerm,'bpm')
            
            st_hLines.LTLines(10)	= line(...
                                    'Xdata',st_longTerm.time,...
    ...
    function fn_control_stagebutton(hObject,~)
        ...
        switch hObject
            ...
            case st_ctrlScore.ABut     
                vt_evId	= size(st_hyp.arousals,1);
                vt_evId	= vt_evId + 1;
                st_hyp.arousals(vt_evId,1)	= st_dat.time{1}(nm_scoreBeg);
                st_hyp.arousals(vt_evId,2)	= st_dat.time{1}(nm_scoreEnd);
                
            case st_ctrlScore.CBut  
                if ~isempty(st_hyp.arousals)
                    vt_evId	= st_hyp.arousals >= st_dat.time{1}(nm_scoreBeg) & ...
                            st_hyp.arousals <= st_dat.time{1}(nm_scoreEnd);
                    vt_evId = sum(vt_evId,2) > 1;    

                    st_hyp.arousals(vt_evId,:)	= [];
                end
            otherwise
